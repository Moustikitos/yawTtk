...
# loads Tile package
def loadTile(tk):
    # append "[pyTtk.py path]/tcl" to AUTO_PATH in order to find Tile package
    # here
...
def _style(widget_or_style):
    return tkinter.BaseWidget.cget(widget_or_style, "style") \
        if isinstance(widget_or_style, tkinter.BaseWidget) else \
        widget_or_style
...
def _img(img, kind="photo", **statespec):
    ...
    if hasattr(img, "tk"):
        ...
        setattr(tkinter._default_root, "PILimg%03d" % _nbPILimg, img)
         ...
         img not in tkinter._default_root.tk.call("image", "names"):
    ...
    for key in ["image", "selectimage", "bitmap"]:
        ...
        if image != None:
            if isinstance(image, tuple) and len(image) == 2:
                ...
                config[key] = \
                    _img(image, kind="photo" if "image" in key else "bitmap")
...
def _dict(Tcl_pairs):
    Tcl_pairs = tuple(str(elem)[1:] if isinstance(elem, _tkinter.Tcl_Obj) \
                else elem for elem in Tcl_pairs)
    return dict(
        (Tcl_pairs[i], Tcl_pairs[i+1]) for i in range(0, len(Tcl_pairs), 2)
    )


_convertto =  lambda value, encoding="utf-8": \
    tkinter._default_root.tk.call("encoding", "convertto", encoding, value)

_options = lambda cnf, **kw: \
    reduce(tuple.__add__, (("-" + k, v) for (k,v) in dict(cnf, **kw).items()))


def getOpenFile(master = None, cnf = {}, encoding="utf-8", **kw):
    return _convertto(
        tkinter._default_root.tk.call(
            ("tk_getOpenFile", "-parent", "." if master == None else master) +
            _options(cnf, **kw)
        ),
        encoding
    )
...
def getSaveFile(master = None, cnf = {}, encoding="utf-8", **kw):
    return _convertto(
        tkinter._default_root.tk.call(
            ("tk_getSaveFile", "-parent", "." if master == None else master) +
            _options(cnf, **kw)
        ),
        encoding
    )
...
def getColor(master = None, cnf = {}, **kw):
    return tkinter._default_root.tk.call(
        ("tk_chooseColor", "-parent", "." if master == None else master) +
        _options(cnf, **kw)
    )
...
class TtkWidget(tkinter.BaseWidget):
    ...
    def configure(self, cnf = {}, **kw):
        ...
        if not len(items):
            return dict(
                ([elem[0][1:], str(elem[-1])] for elem in self.tk.call(self._w, "configure")),
            )
...
class Scrollbar(Widget, tkinter.Scrollbar):
    ...
    def __init__(self, master=None, cnf={}, **kw):
        target = cnf.pop("target", kw.pop("target", False))
    ...
    def configure(self, cnf={}, **kw):
        ...
        if self.__target:
            if not isinstance(self.__target, tkinter.BaseWidget):
                self.__target = searchWidget(
                    self.winfo_toplevel(), self.__target
                )
                if not self.__target:
                    raise tkinter.TclError(
                        'Could not find scrolltarget for "%s" widget' % self
                    )
            try:
                if str(self["orient"]) == "horizontal":
                    config["command"] = self.__target.xview
                    self.__target["xscrollcommand"] = self.set
                else:
                    config["command"] = self.__target.yview
                    self.__target["yscrollcommand"] = self.set
            except Exception as error:
                tkinter.Scrollbar.set(self, lo, hi)
                raise tkinter.TclError(
                    '"%s" is not a scrollable widget' % self.__target
                )
    
        result = Widget.configure(self, **config)
        if isinstance(result, dict):
            result["target"] = \
                ("target", "target", "Widget", False, self.__target)
        return result

    config = configure

    def grid(self, *args, **kw):
        self.__layout = tkinter.Scrollbar.grid
        self.__layout_remove = tkinter.Scrollbar.grid_forget
    ...
    def pack(self, *args, **kw):
        self.__layout = tkinter.Scrollbar.pack
        self.__layout_remove = tkinter.Scrollbar.pack_forget
        self.__layout_args = args
        self.__layout_kw = kw
        self.__layout(self, *self.__layout_args, **self.__layout_kw)
        return self
    ...
    def place(self, *args, **kw):
        self.__layout = tkinter.Scrollbar.place
        self.__layout_remove = tkinter.Scrollbar.place_forget
        self.__layout_args = args
        self.__layout_kw = kw
        self.__layout(self, *self.__layout_args, **self.__layout_kw)
        return self
    ...
    def set(self, lo ,hi):
        if float(lo) <= 0.0 and float(hi) >= 1.0:
            ...
            self.__layout(self, *self.__layout_args, **self.__layout_kw)
        return tkinter.Scrollbar.set(self, lo, hi)
...
class Scrolledframe(Frame):
    ...
    def __init__(self, parent, cnf={}, **kw):
        ...
        Scrollbar(self, orient="vertical", target=self).grid(
            row=0, column=1, sticky="ns"
        )
        Scrollbar(self, orient="horizontal", target=self).grid(
    ...
    def xview(self, *args, **kw):
        return self.canvas.xview(*args, **kw)

    def yview(self, *args, **kw):
        return self.canvas.yview(*args, **kw)

    def configure(self, cnf={}, **kw):
        config = dict(cnf, **kw)
        if hasattr(self, "canvas"):
            self.canvas["xscrollcommand"] = config.pop("xscrollcommand", None)
            self.canvas["yscrollcommand"] = config.pop("yscrollcommand", None)
            self.canvas["background"] = self.inner_frame["background"] = \
                config.get("background", config.get("bg", self.cget("bg")))
        return Frame.configure(self, **config)
